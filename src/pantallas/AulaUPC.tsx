...
import { abrirSesionUPCEnLaptop, revisarAulaUPC } from "../sync/aula";
...
export function AulaUPC({ onBack }: { onBack: () => void }) {
  ...
  const actualizar = async () => {
    ...
    try {
      await revisarAulaUPC();
      cargarEstado();
    } catch (error) {
      setMensaje(error instanceof Error ? error.message : "No se pudo actualizar el aula.");
  ...
  return <div style={{ paddingBottom: 24 }}>
    <Header eyebrow="Universidad" title="Aula UPC" onBack={onBack} right={
      <button className="btn card" onClick={() => void actualizar()} disabled={actualizando} aria-label="Actualizar aula" style={{ padding: 9, display: "flex", color: actualizando ? "var(--ambar)" : undefined }}>
    ...
    {visibles.length === 0 ? <Nota icono={<BookOpenCheck size={16} color="var(--ink2)" />}>No hay elementos para este filtro. Si la laptop está apagada, conservas aquí la última copia recibida.</Nota> : <div style={{ padding: "14px 20px", display: "grid", gap: 8 }}>{visibles.map((item) => <Fila key={item.id} item={item} cursos={cursos} completa />)}</div>}
  </div>;
}
