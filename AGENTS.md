...
| Tema | Decisión |
|---|---|
| Plataforma | **APK Android con Capacitor** (plugins nativos propios en `android/.../app/`), que sigue funcionando como PWA en la web |
...
| Control (tiempo en pantalla) | Módulo privado detrás de la contraseña de Zamly (botón `X` en Hoy). Ver §11 |
...
## 11. Control: tiempo en pantalla y filtro +18 (cerrado, 2026-09-13)

Pedido explícito del dueño: no sabe medir por sí mismo el tiempo en TikTok, Instagram,
Telegram o juegos, y quiere bloquear contenido +18. Es una herramienta que **él se impone**,
no un plan contra el cual se lo mide.

- **Entrada:** botón con ícono `X` en el header de Hoy → contraseña de Zamly → pestañas
  **Racha | Control**. La pulsación larga en "Actualizaciones" ya no existe.
- **Motor nativo** (`android/.../control/`): accesibilidad + UsageStats + pantalla de bloqueo +
...
- **Tono:** pantallas de bloqueo neutras ("TikTok · 1 h de 1 h hoy"), sin rojo, sin culpa.
  Los intentos bloqueados se guardan como dato (dominio o paquete + hora), nunca la URL completa
  ni texto de chats.
- **Coordinación entre agentes:** tablero y encargos en `docs/coordinacion.md`.
