...
// Donde tengas `Capacitor.isNativePlatform()`.
...
export function useReconocedor(
    ...
    if (!texto) {
      setError(
        contesto || !esNativo
          ...
          : "El reconocedor de voz del teléfono no respondió. Comprueba que el APK instalado sea el que trae el plugin.",
      );
    }
  ...
  const alCorte = useCallback(
    (d: CorteNativo) => {
      ...
      if (d.motivo === "permiso") {
        return fallar("Se perdió el permiso del micrófono. Actívalo en Ajustes de Android → TU APP → Permisos.");
      }
  ...
  const iniciar = useCallback(async () => {
    ...
    try {
      ...
      setError(
        causa === "sin-permiso"
          ? "Falta el permiso del micrófono. Actívalo en Ajustes de Android → TU APP → Permisos."
          : causa === "apk-viejo"
            ? "Este APK todavía no trae el dictado del teléfono. Actualiza la app."
